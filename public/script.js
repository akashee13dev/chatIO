    ...
    if (newUser !== localStorage.getItem("username")) {
      showPopup(newUser , true);
        ...
        for (let userName of users) {
            ...
            li.className = "bg-gray-700 px-4 py-2 rounded-lg shadow";//+ (userName == user ? " text-green-500" : "");
            if (user === userName) {
                li.classList.add("font-bold", "bg-green-800");  // darker green background & bold text
                li.textContent += " (You)";
              }
            li.id = "user " + userName;
